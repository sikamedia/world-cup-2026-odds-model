#!/usr/bin/env python3
"""Football-odds-model backtest — ALL 44 completed 2026 WC matches (through Jun 23).

Extends the prior 40-game backtest with the 4 new matchday-2 games of Groups I & J
played Jun 22 (France-Iraq, Norway-Senegal, Argentina-Austria, Algeria-Jordan).
Those 4 are a genuine OUT-OF-SAMPLE test of v3.3, whose 3 fixes were tuned on the
original 40 (in-sample). We therefore report:
  (a) full-44 metrics for v3.2 and v3.3,
  (b) the 4 new games in isolation (out-of-sample),
  (c) candidate v3.4 improvement sweeps on the full 44.

Self-contained (no imports) so it doubles as a reproducible deliverable.
Engine math is identical to match_model.py / match_model_v33.py.
Educational/analytical use only — not betting advice.
"""
import math

# ---- Elo (same snapshot that produced the validated 40-game numbers) ----
ELO = {
...
HOME = 85  # host-nation true-home Elo bump (Mexico/USA/Canada home games)

# (home, away, hg, ag, host_home, is_new_out_of_sample)
G = [
 ...
 # ---- 4 NEW games (Jun 22), out-of-sample for v3.3 ----
...
def predict(h_, a_, ho, cfg):
    """Return (ph,pd,pa,pov, P) for a game under a config dict."""
...
def metrics(games, cfg):
    ...
    for h_,a_,hg,ag,ho,new in games:
        ...
        r=res(hg,ag)
        am = 0 if ph>pa else 2                 # H/A-only argmax (legacy)
        arg3 = max(range(3), key=lambda k:[ph,pd,pa][k])  # true 3-way argmax
        acc_hada += (am==r)
        acc_arg3 += (arg3==r)
...
V32 = dict(avg_goals=2.6, auto_open=False, open_delo=300, draw_boost=0.06, draw_gate=0.52)
V33 = dict(avg_goals=2.8, auto_open=True,  open_delo=300, draw_boost=0.06, draw_gate=0.42)
...
allg = G
new4 = [g for g in G if g[5]==1]
old40 = [g for g in G if g[5]==0]

print("#"*64)
print("# PART 1 — v3.2 vs v3.3 on the FULL 44 played matches")
print("#"*64)
show("v3.2 (avg_goals 2.6, no auto-open, gate 0.52) — FULL 44", metrics(allg,V32))
show("v3.3 (avg_goals 2.8, auto-open>=300, gate 0.42) — FULL 44", metrics(allg,V33))

print("\n"+"#"*64)
print("# PART 2 — OUT-OF-SAMPLE: the 4 new Jun-22 games only")
print("#   (v3.3 was tuned on the other 40; this is the honest test)")
print("#"*64)
mN32=metrics(new4,V32); mN33=metrics(new4,V33)
print("\nPer-match (v3.3):")
print(f"{'Match':28}{'Score':>6}{'  H/D/A model%':>18}{'  argmax':>8}{'topCS':>7}{'P(act)':>8}{'O/U':>5}")
for (h_,a_,hg,ag,ph,pd,pa,pov,top,pact,new) in mN33["rows"]:
...
show("v3.2 — NEW 4 only", mN32)
show("v3.3 — NEW 4 only", mN33)

print("\n"+"#"*64)
print("# PART 3 — candidate v3.4 improvement sweeps on FULL 44")
print("#"*64)

print("\n[3a] draw_boost sweep (re-confirm optimum on 44)")
print(f"{'draw_boost':>10}{'RPS':>9}{'logL':>9}{'modelDraw%':>12}")
for db in [0.0,0.03,0.06,0.09,0.12]:
    c=dict(V33); c["draw_boost"]=db; m=metrics(allg,c)
    print(f"{db:>10.2f}{m['rps']:>9.4f}{m['ll']:>9.2f}{m['draw_prob']*100:>11.1f}%")

print("\n[3b] auto-open ΔElo threshold sweep (blowout tail)")
print(f"{'thresh':>10}{'logL':>9}{'blowExp':>9}{'blowAct':>9}{'RPS':>9}")
for thr in [99999,400,350,300,250,200]:
    c=dict(V33); c["open_delo"]=thr; c["auto_open"]=(thr<99999); m=metrics(allg,c)
...
for ag_ in [2.6,2.7,2.8,2.9,3.0]:
    c=dict(V33); c["avg_goals"]=ag_; m=metrics(allg,c)
    povavg=sum(row[7] for row in m["rows"])/m["n"]
    print(f"{ag_:>10.1f}{povavg*100:>11.1f}%{m['brier_ou']:>9.4f}{m['rps']:>9.4f}")

print("\n[3d] gd_per_100 sweep (Elo->goal-diff slope; favourites separation)")
print(f"{'gd/100':>10}{'RPS':>9}{'logL':>9}{'WDL H/A%':>10}")
for gd in [0.40,0.45,0.50,0.55]:
    c=dict(V33); c["gd_per_100"]=gd; m=metrics(allg,c)
    print(f"{gd:>10.2f}{m['rps']:>9.4f}{m['ll']:>9.2f}{m['acc_hada']/m['n']*100:>9.0f}%")

print("\n[3e] worst-calibrated scorelines (lowest model P on actual), v3.3 full-44")
worst=sorted(metrics(allg,V33)["rows"], key=lambda x:x[9])[:10]
