...
KO_RESULTS: list[tuple[str, str, int, int, str, str]] = [
    ...
    ("France", "Spain", 0, 2, "A", "SF"),           # SF1 Dallas 7/14. Oyarzabal pen 22', Porro 58'. Spain to FINAL. Model-market divergence case resolved MODEL-side: model SPA adv 52.9 vs FanDuel-implied FRA 57.8 (10.7pt gap) AND 90' model Spa 37.2 vs mkt 30.1 — Spain won in 90'. NO official artifact (16:05Z run FAIL CLOSED, Elo egress blocked; preview-only numbers). Verified ESPN/FIFA/NBC/NPR/AlJazeera 7/15.
    ("England", "Argentina", 1, 2, "A", "SF"),      # SF2 Atlanta 7/15. Gordon 55' (Rogers cross); Enzo Fernandez 85' long-range, L.Martinez 90+2' header — BOTH assisted by Messi. Argentina to FINAL vs Spain. Divergence case #6 resolved MODEL-side: model ARG adv 59.1 vs FanDuel-implied ENG 53.7 (12.8pt gap) AND 90' model Arg 44.9 vs mkt ~31.1 — Argentina won in 90'. Draw alert 32.0 did NOT fire. NO official artifact (16:05Z FAIL CLOSED, Elo+weather 403; 17:07-17:24Z user-session evidence captured pre-match). Verified ESPN/FIFA/NBC/NPR/AlJazeera 7/16.
    ("France", "England", 4, 6, "A", "3P"),         # 3P103 Miami 7/18, 90' 4-6 ("10-goal thriller"; England survived a France rally to take bronze). England ROTATED XI (Kane/Bellingham/Pickford benched, Toney/Rashford/Eze started); France near-full (Mbappé started, Deschamps farewell, Golden Boot chase). Heat 89°F feels-like ~101 at KO. FIRST official schema-4 artifact (4ac75a42): official 90' Fra 43.4/30.5/26.1, adv Fra 58.1/Eng 41.9 — both model and ensemble favoured France; ENGLAND won → divergence case (10.8pt 90' Eng-side vs DK) resolved on the ENGLAND/market-gap side but model's Eng-lean vs market was directionally right; blowout tail 10 goals = extreme O/U miss for both. Elo swing Eng 2074→2125, Fra 2121→2070. Verified ESPN/FOX/FIFA/Yahoo 7/19.
    ("Spain", "Argentina", 0, 0, "H", "F"),         # F104 FINAL MetLife 7/19, 90' 0-0 -> SPAIN 1-0 AET (Ferran Torres 106'); Enzo Fernandez RED 90+3 (2nd yellow, foul on Cubarsi) — Argentina played ET a man down. E.Martinez heroics kept 90' scoreless (Spain dominated). SECOND official schema-4 artifact (7e4ac9ad): official 90' Spa 39.7/32.3/28.0 (draw = 2nd-most-likely, RIGHT), adv Spa 55.5/Arg 44.5; model adv Spa 53.5 vs Kalshi direct two-way 58.5 (5.0pt Arg-side gap, case #7) -> SPAIN won: FIRST market-side divergence resolution (model streak ends 6/7). Spain champions (2nd title). Verified FIFA/ESPN/FOX/Yahoo 7/20.
]
