#!/usr/bin/env python3
"""Football-odds-model backtest — ALL 54 completed 2026 WC matches (through Jun 24).

Extends backtest_48.py with the 6 Group A/B/C matchday-3 games played Jun 24:
  Mexico 3-0 Czechia, South Africa 1-0 South Korea (Group A);
  Canada 1-3 Switzerland, Bosnia 3-1 Qatar (Group B);
  Brazil 3-0 Scotland, Morocco 4-2 Haiti (Group C).
These 6 are a genuine OUT-OF-SAMPLE test of v3.5 (whose gd_per_100=0.60 fix was
tuned on the prior 48). Shared tournament data now lives in
worldcup_2026_data.py. We report:
  (a) full-54 metrics for v3.4 / v3.5,
  (b) the 6 new Jun-24 games in isolation (out-of-sample for v3.5),
  (c) candidate v3.6 sweeps on the full 54.

Engine math identical to match_model_v35.py.
Educational/analytical use only — not betting advice.
"""
import math

from worldcup_2026_data import ELO, HOME, MATCHES_54 as G
...
def elo_to_lambdas(elo_h, elo_a, home_bump=0.0, avg_goals=2.85, gd_per_100=0.60):
...
def predict(h_, a_, ho, cfg):
    ...
    lh, la = elo_to_lambdas(eh, ea, avg_goals=cfg["avg_goals"],
                            gd_per_100=cfg.get("gd_per_100",0.60))
    gap = eh - ea
    style = "balanced"
...
allg = G
new6 = [g for g in G if g[5]==3]   # Jun-24, out-of-sample for v3.5

print("#"*64)
print("# PART 1 — v3.4 / v3.5 on the FULL 54 played matches")
print("#"*64)
show("v3.4 (gd .55, db .07) — FULL 54", metrics(allg,V34))
show("v3.5 (gd .60, db .08) — FULL 54", metrics(allg,V35))

print("\n"+"#"*64)
print("# PART 2 — OUT-OF-SAMPLE: the 6 new Jun-24 games only")
print("#   (v3.5's gd=0.60 fix was tuned on the prior 48; honest test)")
print("#"*64)
mN34=metrics(new6,V34); mN35=metrics(new6,V35)
print("\nPer-match (v3.5 pre-match prediction):")
print(f"{'Match':30}{'Score':>6}{'  H/D/A model%':>18}{'argmax':>7}{'topCS':>7}{'P(act)':>8}")
for (h_,a_,hg,ag,ph,pd,pa,pov,top,pact,new) in mN35["rows"]:
    r=res(hg,ag); arg3=max(range(3),key=lambda k:[ph,pd,pa][k])
    hit='OK' if arg3==r else 'X'
    print(f"{h_+' v '+a_:30}{str(hg)+'-'+str(ag):>6}{ph*100:6.0f}/{pd*100:3.0f}/{pa*100:3.0f}%"
          f"{hit:>7}{str(top[0])+'-'+str(top[1]):>7}{pact*100:7.1f}%")
show("v3.4 — NEW 6 (Jun24) only", mN34)
show("v3.5 — NEW 6 (Jun24) only", mN35)

print("\n"+"#"*64)
print("# PART 3 — candidate v3.6 sweeps on FULL 54 (vs v3.5 baseline)")
print("#"*64)

print("\n[3a] gd_per_100 sweep (is the optimum still drifting up?)")
print(f"{'gd/100':>10}{'RPS':>9}{'logL':>9}{'modelDraw%':>12}{'blowExp':>9}{'argmax':>8}")
for gd in [0.50,0.55,0.60,0.62,0.65,0.70]:
    c=dict(V35); c["gd_per_100"]=gd; m=metrics(allg,c)
...
for db in [0.05,0.07,0.08,0.10,0.12]:
    c=dict(V35); c["draw_boost"]=db; m=metrics(allg,c)
...
for ag_ in [2.7,2.8,2.85,2.9,3.0,3.1]:
    c=dict(V35); c["avg_goals"]=ag_; m=metrics(allg,c)
    povavg=sum(row[7] for row in m["rows"])/m["n"]
    print(f"{ag_:>10.2f}{povavg*100:>11.1f}%{m['brier_ou']:>9.4f}{m['rps']:>9.4f}")

print("\n[3d] worst-calibrated scorelines (lowest model P on actual), v3.5 full-54")
worst=sorted(metrics(allg,V35)["rows"], key=lambda x:x[9])[:12]
for (h_,a_,hg,ag,ph,pd,pa,pov,top,pact,new) in worst:
    tag={0:'',1:' [oos-Jun22]',2:' [oos-Jun23]',3:' [oos-Jun24]'}[new]
