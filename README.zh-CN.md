...
- Release preparation 工具和只允许从 `main` 发布的 GitHub prerelease workflow。
...
|-- release_tool.py                 (版本准备与 release 一致性校验)
|-- football-odds-model.skill
|-- .github/workflows/              (PR CI 与 main-only release workflow)
|-- archive/                        (冻结的历史模型 + 回测)
`-- test_*.py
```
...
开发分支：`dev`；合并到 `main` 后再从 `main` 创建 release tag。
...
先在 `dev` 自动准备版本元数据和仓库内的 skill bundle：

```bash
python3 release_tool.py prepare v0.4.0-rc.1
```

提交生成的修改，并通过 PR 将 `dev` 合入 `main`。随后从 `main` 手动运行 GitHub
Actions 的 `Release` workflow，并输入同一个 tag。workflow 会先验证元数据、测试和
bundle，再创建 annotated tag；RC tag 会生成 GitHub prerelease，并附加 `.skill`
和 SHA-256。workflow 不会在 `main` 上创建版本提交。
