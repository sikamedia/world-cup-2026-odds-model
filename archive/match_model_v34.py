#!/usr/bin/env python3
"""Single-match bookmaker model — v3.4 (backtest-improved, 44-game).

Pure standard library. This is v3.3 plus ONE new fix validated on all 44 played
2026 WC matches (40 in-sample + 4 out-of-sample Jun-22 games):

  FIX 4 — Steeper Elo->goal-difference slope. `gd_per_100` 0.45 -> 0.55. The old
          0.45 (a generic club-football regression value) under-separated
          favourites from underdogs in a 48-team field with big talent gaps. The
          2026 data wants ~0.55-0.60. Backtest (full 44): RPS 0.1609->0.1572,
          scoreline logL -126.4->-125.1, blowout expectation 8.3->9.7 (actual 12).
          Improvement holds BOTH in-sample (RPS 0.1634->0.1609) and out-of-sample
          (RPS 0.1359->0.1202), so it is a real effect, not overfitting. It also
          fixes the blowout under-count more cleanly than the negbin tail alone.
...
GD_PER_100_DEFAULT = 0.55   # FIX 4 (was 0.45): steeper favourite separation
...
def main():
    ap = argparse.ArgumentParser(description="Bookmaker single-match model v3.4")
    ...
    ap.add_argument("--gd-per-100", type=float, default=GD_PER_100_DEFAULT,
                    ...
                    help=f"goal-diff per 100 Elo (default {GD_PER_100_DEFAULT}; "
                         f"FIX 4, was 0.45 — favourites separate more in a 48-team field)")
